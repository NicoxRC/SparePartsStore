...
The core invoicing document — what replaces a manually-issued paper/PDF invoice. Corresponds to Dataico's "1. Factura electrónica estándar" collection. **Field-level shape (line items, tax breakdown, required customer fields, response format) is pending** — see `docs/phases/PHASE_9_INVOICING_STANDARD.md`.

### Nota crédito / Nota débito (Credit note / Debit note)
Adjustments issued against an already-sent invoice — a credit note reduces what's owed (return, discount, error correction), a debit note increases it (an additional charge). Both live under the same "Factura electrónica estándar" Dataico collection as the invoice itself. Pending exact API shape, same as above.

### Resolución DIAN / DIAN resolution (numbering authorization)
DIAN authorizes a business to issue invoices only within a specific numbering range and validity window ("resolución de facturación"), per document type. An invoice sent outside its resolution's authorized range/date is invalid. Corresponds to Dataico's "8. Actualizar o vincular resoluciones" collection — this is why it's planned as an **early** invoicing phase (`PHASE_8_RESOLUTIONS.md`): an invoice can't legitimately be sent before a resolution is on file. Exact API mechanics pending that module's reference.

### CUFE (Código Único de Facturación Electrónica)
The unique code DIAN/the provider assigns to a validated electronic invoice — the legal proof-of-existence identifier a business must be able to retrieve and, in some cases, print (e.g. as a QR code) on the invoice representation. Exact response field Dataico returns it under is pending the invoicing-standard module's reference.

### Tercero (Third party) / Consulta DIAN Terceros
"Tercero" is the generic DIAN term for the other party in a transaction — here, the invoice's customer/recipient. Dataico's "7. Consulta DIAN Terceros" collection is a lookup service (almost certainly by NIT/cédula) used to validate or pre-fill a customer's legal identification data before issuing an invoice against them. Exact request/response shape pending that module's reference.
