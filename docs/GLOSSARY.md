...
- `counted_cash`/`cash_discrepancy` — what the cashier actually counts at close time, and the gap against `expected_cash`. If it doesn't square, an admin can correct `counted_cash` afterward (`PATCH /api/cash-register/:id/counted-cash`) without reopening anything else about that day — the discrepancy is always shown, never hidden.
