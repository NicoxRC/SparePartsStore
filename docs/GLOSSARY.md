...
The core invoicing document — what replaces a manually-issued paper/PDF invoice. Corresponds to Dataico's "1. Factura electrónica estándar" collection. **The "Envío Factura" (send) request shape is confirmed** — see `docs/phases/PHASE_10_INVOICING_STANDARD.md` for the full payload. Its response shape, and the resend/query requests, are still pending.
