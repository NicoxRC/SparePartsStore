...
A small enhancement connecting Phases 10 and 12, **not a Dataico integration** — a persisted local "customer address book" (`customers` table) so store staff can search and reuse a customer across sales instead of retyping or re-looking-up every time. Created/edited directly by staff (admin or employee) via `CustomersService`, `POST/GET/PATCH /api/customers` — zero HTTP calls to Dataico. Distinct from **Tercero** above: "Tercero" is the live, uncached DIAN identity-validation lookup from Phase 9, while `customers` is this app's own reusable, persisted record — the two are not linked by foreign key, and `invoices`/`pos_invoices` still carry their own denormalized `customer_*` columns untouched by this table. Also stores a NIT check digit (`identification_dv`) and `phone`, both **local-only** — neither is part of the confirmed Dataico standard-invoice `customer` payload, so they're never sent to Dataico, only kept here and shown on the invoice form for this store's own record. See `docs/DATABASE.md` for the schema.

### Caja / Apertura y cierre de caja (Cash register open/close)
A local-bookkeeping enhancement, **not a Dataico integration** — one shared cash register per calendar day for the whole store (not per user or per session), tracked in `cash_registers`. Opening ("abrir caja") is **mandatory before any new invoice can be created**: `InvoicesService.create` calls `CashRegisterService.assertOpenToday()` and refuses with a 400 if no register is open for today — this gate applies only to creating new invoices, not to viewing invoice history or to resend/refresh actions on an existing one. Closing ("cerrar caja") at the end of the day **auto-calculates** the day's total as the sum of that day's `invoices.total_amount` — there is no manual total-entry field. "Today" and the daily total are computed against the store's local calendar day (`America/Bogotá`), not server time (Railway runs UTC) — see `docs/DATABASE.md`. There is no reopen flow — once closed, a day's register is done. Per-seller breakdown isn't tracked here — that's already covered by each invoice's own `created_by_id`. In code: `CashRegisterService`, `POST /api/cash-register/open`, `POST /api/cash-register/close`, `GET /api/cash-register/today`, `cash_registers` table.
