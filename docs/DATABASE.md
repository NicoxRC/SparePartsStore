...
| `borrower_type` | VARCHAR(20), default `almacen`, indexed | `almacen` (a business, full invoice data required) or `empleado` (a person, name only). Validated with `@IsIn` at the DTO layer, not a DB enum. An `empleado` row leaves every invoice-data column below null. See `GLOSSARY.md`'s "Cotización". |
| `customer_identification_type`, `customer_identification`, `customer_identification_dv`, `customer_party_type`, `customer_tax_level_code`, `customer_regimen`, `customer_company_name`, `customer_first_name`, `customer_family_name`, `customer_country_code`, `customer_department`, `customer_city`, `customer_address_line`, `customer_email`, `customer_phone` | VARCHAR, all nullable (since migration 34; the service still requires the invoice data for `almacen`) | Same shape as `invoices`' own `customer_*` columns, denormalized the same way — plus `customer_identification_dv`/`customer_phone`, which `invoices` doesn't persist (Dataico doesn't need them) but a quotation, never sent to Dataico directly, keeps for its own record. |
| `notes` | TEXT, nullable | |
...
| `unit_price` | NUMERIC(12,2) | **The locked price** — a snapshot of `products.sale_price` (the IVA-included price — the quotation's total is just the sum of these, IVA is only broken out for display) taken when the product is first added to the quotation, confirmed with the human: the customer keeps the price they were quoted, even if the product's price changes before they come back to pay. |
...
| 34 | `AddBorrowerTypeToQuotations` | Adds `quotations.borrower_type` (default `almacen`, so every existing row is an almacén one) with an index, and drops `NOT NULL` from the invoice-data `customer_*` columns so an `empleado` quotation can leave them empty. `down()` restores `NOT NULL`, which fails while an empleado quotation without that data exists. |
