...
**`tax_base` is not the amount in pesos** (corrected 2026-09-21 — this used to be recorded here as an "anomaly, not replicated"): the confirmed example sends `tax_base: 100` on every item, and its `tax_amount` (`3782628`) is exactly 19% of price × quantity (`281 × 70849 = 19,908,569`), so the example was consistent all along. Dataico confirmed it the hard way: sending the computed base made `POST /invoices` fail with `El numero '121849' tiene que estar entre 1 y 100` on `invoice.items[0].taxes[0].tax_base` — a validation error that Dataico returned as **HTTP 500**, which this app surfaces as 502. So `tax_base` is always `100` (the whole line is taxable) and the real amounts travel in `price`, `quantity` and `tax_amount` — see `DATAICO_TAX_BASE` in `InvoicesService`. Credit/debit notes never sent `tax_base`, so they are unaffected.

**Test/dry-run mechanism confirmed**: controlled via `actions.send_dian: false`, not a special `env` value — only `"PRODUCCION"` has been seen for `env`. The app used to always send `send_dian: true`; it now reads it (and `send_email`) from `DATAICO_SEND_DIAN` / `DATAICO_SEND_EMAIL`, both **off by default** — see `docs/ENVIRONMENT_VARIABLES.md`. `env` is still hardcoded `"PRODUCCION"`.
...
- [x] Frontend: "Nota débito" action on `InvoicesListPage` opening a dedicated form (pick products, quantities — tax rate is derived from each product, not typed) against that invoice; a `DebitNotesListPage` linked from Facturas to see issued notes.
...
- **Single, hardcoded tax category (IVA).** No retentions — matches "Estructura básica," not the more complex variants this store doesn't need. A product with no IVA is sent as `tax_rate: 0` under the same `IVA` category (the only one confirmed) — the client marks this as "Exenta" purely as a local UI label, no separate field is sent to Dataico for it. **IVA is never client-editable per line anymore** — confirmed directly: every product always charges the standard 19% unless its own `tax_exempt` flag says otherwise, so `resolveTaxRate()` derives it from `Product.taxExempt` and `taxRate` was removed from every item DTO entirely (see `docs/DATABASE.md`'s `products` table).
- **A per-line fixed discount exists, but purely as a local calculation — not a Dataico invoice-level charge/discount field.** `CreateInvoiceItemDto.discount` (a flat COP amount, not a percentage) is subtracted from that line's final, IVA-included amount (`price × quantity`), then the IVA is unwrapped from what's left; the discounted amount is folded into the `price`/`tax_base`/`tax_amount` this app already computes and sends, so `price × quantity` on the actual invoice already equals the discounted pre-tax base. Dataico never sees a "discount" field — see `InvoicesService.resolveItems()`. As of the "Aplicar descuento" UI change, this per-line value is no longer typed by hand — the client prorates one whole-sale discount percentage (or COP value) across every line before submitting (see `docs/DATABASE.md`'s `quotation_items` note); the field itself, and everything downstream of it, is unchanged.
- **`Product.salePrice` already includes IVA** (decided 2026-09-21, after briefly being "IVA on top"): `resolveItems()` subtracts the discount above from `price × quantity` and unwraps the IVA out of what's left, sending Dataico the pre-tax `price` — see `GLOSSARY.md`'s "Sale price" entry. Existing invoices are unaffected: each stores the exact request it sent.
...
- **`number` auto-increment is a simple `MAX()` read, not a race-proof counter.** `InvoicesService.resolveNextNumber()` reads the highest locally-recorded `number` for the active resolution's prefix and adds one (or `INVOICE_NUMBER_START` if that's higher — see `docs/ENVIRONMENT_VARIABLES.md`). Two concurrent creates could theoretically compute the same next number; accepted as a low-probability edge case at this store's scale rather than built around with row-locking.
