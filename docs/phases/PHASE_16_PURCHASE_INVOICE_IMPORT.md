...
- **Price on an existing product is an optional price *change*** (superseding the first version, which ignored it): a row/line that matches an existing product and carries a different price updates the product's `sale_price` on confirm; blank keeps the current one. Same for a "new" line that turns out to exist at confirm time (D5) — its typed price applies. See "Rule added later" below.
...
## Rule added later: an existing product can get a new sale price

A price may have changed since the last purchase, so a line linked to an existing product may carry a **new sale price**, shown in the review as `precio actual → precio nuevo` next to the stock's `stock N → N+qty`. Blank keeps the current price. On confirm the product's `sale_price` is updated (with `updated_by`), the change is written into the movement's notes (`Precio de venta: $10.000 → $12.500` — the only price history that exists) and returned in the confirm response (`pricesUpdated`, `priceChanges[]`). A typed price must be an integer ≥ 500 (`INVALID_SALE_PRICE`, also on linked lines). The description of an existing product is still never touched. Because this lets a purchase change prices, `purchase_imports.confirm` should be granted deliberately (it already creates products and moves stock).

**Consequence that had to be fixed: a sale's price must not change when the product's does.** Verified while adding this: *invoices* were already safe (they store the exact request sent); *quotations* were **not** — editing one re-priced every line at the product's current price, so a later price change silently altered old quotations (fixed: existing lines keep the quoted price); *credit notes* used the product's current price (fixed: they use the price/IVA of the original invoice line, read from its stored request; `InvoiceResponseDto.items` now exposes those lines so the client preview matches). **Open decision, left as it was:** *debit notes* still use the product's current price — they are a new charge issued today — say if they should follow the original invoice instead. Not covered: a credit note for a product whose `reference` was changed after the sale won't find its invoice line (matched by `sku`) and falls back to the current price.
