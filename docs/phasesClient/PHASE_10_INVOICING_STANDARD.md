...
- **`InvoiceFormPage`** (`/ventas`) — the main deliverable:
...
- Nav has two entries for admin **and employee** (not auditor), matching the backend's role tier since this is the everyday counter-sale action, not an admin-only configuration screen: **"Venta"** (`/ventas`) is the only place a new invoice gets created; **"Facturas"** (`/invoicing/invoices`) is the read-only list for consulting status, resending, and (later) notes — it has no "new invoice" entry point.
