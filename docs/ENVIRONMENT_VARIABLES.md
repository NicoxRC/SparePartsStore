...
| `INVOICE_NUMBER_START` | ✅ (default `1`) | `InvoicesService` auto-increments the invoice `number` from the highest one already recorded locally for the active resolution's prefix — the next number is `max(last local number + 1, INVOICE_NUMBER_START)`. It seeds the starting value for the gap between the store's real, pre-existing DIAN numbering history and this app's empty local `invoices` table, and can be raised later to jump the local count ahead (e.g. when invoices were issued outside this app on the same Dataico account, as happens during testing). Lowering it has no effect once local invoices exist past it. |
