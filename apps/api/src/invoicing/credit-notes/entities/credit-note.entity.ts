...
/**
 * A local record of a "nota crédito" sent to Dataico — a return/reduction
 * against an already-sent `Invoice` (a returned product, an overcharge,
 * an error correction), referencing it by Dataico's own `dataico_uuid`.
 * See docs/phases/PHASE_10_INVOICING_STANDARD.md ("Still not confirmed")
 * for why this was blocked until a clean example was found, and
 * CreditNotesService for the confirmed request shape.
 *
 * Unlike `customers`/`invoices`, this table is brand new with no legacy
 * rows to reconcile, so it FKs straight to `invoices.id` — the credit
 * note's own `customer`/`payment_means`/`payment_means_type` fields are
 * read from that invoice's stored `request_payload` rather than
 * re-collected, so no `customer_*` columns are duplicated here either.
 *
