...
export function DebitNotesListPage() {
  ...
  const debitNotesQuery = useDebitNotes({ page, limit: PAGE_SIZE });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {debitNotesQuery.isPending && <Spinner label="Cargando…" />}

      {debitNotesQuery.isError && (
        <Alert variant="error">{getApiErrorMessage(debitNotesQuery.error)}</Alert>
      )}

      {debitNotesQuery.data && (
        <>
          {debitNotesQuery.data.data.length === 0 ? (
            <Alert variant="info">Todavía no se ha emitido ninguna nota débito.</Alert>
          ) : (
            <div className="divide-y divide-line border border-line bg-paper">
              {debitNotesQuery.data.data.map((note) => (
                <div
                  key={note.id}
          ...
          <Pagination meta={debitNotesQuery.data.meta} onPageChange={setPage} />
        </>
      )}
    </div>
  );
